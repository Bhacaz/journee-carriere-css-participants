...
CSV.open('./data/data.csv', 'r', headers: true) do |csv|
  csv.each do |row|
    participants << {
      ...
      period: row["À quel(s) moment(s) seriez-vous disponible pour animer votre atelier/conférence le 22 novembre ? (atelier de 45 minutes) *Vous pouvez cocher plusieurs cases"],
